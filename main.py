...
def select_menu_item(menu: list[str]) -> str:
    ...
    while True:
        ...
        if user_input.isdigit() and 1 <= int(user_input) <= len(menu):
            ...
            return menu[n - 1]
        print(f"Доступны команды: ", ', '.join(f'{i + 1}. {f}' for i, f in enumerate(menu)))
    ...
    if check_user_desc == 'да':
        ...
        if search_word:
            filtered_transactions = [
                trans for trans in filtered_transactions
                if search_word in str(trans.get('description', '')).lower()
            ]
